...
@triton.jit
def _fused_qk_rope_reshape_and_cache_tiled_kernel(
    q_ptr,
    k_ptr,
    v_ptr,
    pos_ptr,
    ...
    X_SIZE: tl.constexpr,
    FLASH_LAYOUT: tl.constexpr,
    HEADS_PER_BLOCK: tl.constexpr = 8,
    HAVE_POS: tl.constexpr = False,
    HAVE_K_SCALE: tl.constexpr = False,
    HAVE_V_SCALE: tl.constexpr = False,
    HAVE_ZEROS: tl.constexpr = False,
):
    """Tiled variant: each block processes HEADS_PER_BLOCK consecutive Q-heads
    for one token, reducing total blocks from T*QH to T*(QH/HEADS_PER_BLOCK).
    This dramatically cuts kernel dispatch overhead for small decode batches.
    """
    tl.assume(q_stride_t >= 0)
    tl.assume(q_stride_h >= 0)
    tl.assume(q_stride_d >= 0)
    ...
    NUM_Q_GROUPS: tl.constexpr = QH // HEADS_PER_BLOCK

    d_pe_offs = tl.arange(0, BLOCK_D_pe).to(tl.int64)

    # Region 1: Q-head processing (tiled: HEADS_PER_BLOCK heads per block)
    if pid < T * NUM_Q_GROUPS:
        pid_t = pid // NUM_Q_GROUPS
        pid_group = pid % NUM_Q_GROUPS
        pid_hq_base = pid_group * HEADS_PER_BLOCK

        # Load position and cos/sin ONCE per token (shared across all heads in block)
        if REUSE_FREQS_FRONT_PART:
            if IS_NEOX:
                d_cos_offs = d_pe_offs
                d_cos_offs = tl.where(
        ...
        # Pre-load K/V scale if needed (shared across heads)
        if HAVE_K_SCALE:
            k_scale = tl.load(k_scale_ptr)
        else:
            k_scale = 1
        ...
        # Process HEADS_PER_BLOCK consecutive Q-heads
        for h_offset in tl.static_range(HEADS_PER_BLOCK):
            pid_hq = pid_hq_base + h_offset
            ...
            # K/V cache write (every QH_PER_KH-th head)
            if pid_hq % QH_PER_KH == 0:
                pid_slot = tl.load(slot_mapping_ptr + pid_t).to(tl.int64)
                if pid_slot >= 0:
                    pid_t_slot = pid_slot // BLOCK_SIZE
                    ...
                    tl.store(k_cache_ptrs, k_pe.to(key_cache_ptr.dtype.element_ty))
        ...
        # Region 2: Extra K/V tokens (for extend where T_slot > T)
        pid = pid - T * NUM_Q_GROUPS + T * KH
        if pid < T_slot * KH:
            pid_t = pid // KH
            pid_hk = pid % KH
            pid_slot = tl.load(slot_mapping_ptr + pid_t).to(tl.int64)
